...
export const getErrorSignature = (error: string): string | null => {
  const errors = [
  ...
  for (let errSig of errorSignatures) {
    try {
      iface.decodeErrorResult(
        errSig,
        error
      );
      return errSig;
...
export const getErrorMessage = (errSig: string): string => {
