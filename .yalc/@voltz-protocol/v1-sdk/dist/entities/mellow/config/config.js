        ...
        MELLOW_VAULTS: [
            {
                ...
                metadata: {
                    title: 'MELLOW - ETH',
                    token: 'ETH',
                    description: 'This vault is no longer accepting deposits. Funds will become available for withdrawal at pool maturity.',
                    show: true,
                    soon: false,
                    deprecated: true,
                    vaults: [
...
var getConfig = function (_a) {
    ...
    var allNetworks = Object.keys(networkConfigurations);
    if (!allNetworks.includes(network)) {
        throw new Error("Network ".concat(network, " not found in configuration networks ").concat(allNetworks.toString(), "."));
    }
    var config = networkConfigurations[network];
    config = (0, utils_1.disableMaturedWeights)(config);
    var provider = ethers_1.providers.getDefaultProvider(providerURL);
    return __assign(__assign({}, config), { PROVIDER: provider });
};
