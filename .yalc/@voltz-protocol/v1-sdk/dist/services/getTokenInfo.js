...
var getTokenInfo = function (tokenAddress) {
    ...
    // goerli
    if (tokenAddress.toLowerCase().includes('0x79C950C7446B234a6Ad53B908fBF342b01c4d446'.toLowerCase())) {
        return { name: 'USDT', decimals: 6 };
    }
    // goerli
    if (tokenAddress.toLowerCase().includes('0xC2C527C0CACF457746Bd31B2a698Fe89de2b6d49'.toLowerCase())) {
        return { name: 'USDT', decimals: 6 };
    }
    sentry_1.sentryTracker.captureMessage("Token address ".concat(tokenAddress, " not supported."));
    throw new Error("Token address ".concat(tokenAddress, " not supported."));
};
