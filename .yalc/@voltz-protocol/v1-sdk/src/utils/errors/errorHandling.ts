...
export const decodeInfoPostMint = (error: any, environment: string): RawInfoPostMint => {
  ...
  sentryTracker.captureException(getReadableErrorMessage(error, environment));
  sentryTracker.captureMessage(getReadableErrorMessage(error, environment));
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const decodeInfoPostSwap = (error: any, environment: string): RawInfoPostSwap => {
  const errSig = getErrorSignature(error, environment);
