...
export const formatDate = (date: Date) => {
  return `${date.getDay().toString().padStart(2, '0')} ${shortMonths[date.getMonth()]} ${date.getFullYear()}`
}
...
export const formatDateTime = (dateTime: DateTime) => {
  return `${dateTime.day.toString().padStart(2, '0')} ${shortMonths[dateTime.month]} ${dateTime.year}`
}
