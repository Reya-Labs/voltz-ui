...
describe('confirmSwapThunkHandler', () => {
  ...
  it('should call amm.swap and pushSwapTransactionSubmittedEvent with correct event params', async () => {
    ...
    mockState.deprecatedSwapForm.amm.signer.getAddress.mockResolvedValue(mockAccount);
    ...
    expect(mockState.deprecatedSwapForm.amm.swap).toHaveBeenCalledWith({
      isFT: mockIsFT,
      notional: mockNotional,
      margin: mockMargin,
      fixedLow: 1,
  ...
  it('should call pushSwapTransactionFailedEvent with correct event params if unsuccessful', async () => {
    ...
    mockState.deprecatedSwapForm.amm.signer.getAddress.mockResolvedValue('0x123');
    (mockState.deprecatedSwapForm.amm.swap as jest.Mock).mockRejectedValue(mockError);
