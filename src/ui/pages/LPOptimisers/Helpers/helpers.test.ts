import { getSpareWeights } from './getSpareWeights';
...
describe('getSpareWeights', () => {
  ...
  it('non-zero weights with more elements', () => {
    const spareWeights = getSpareWeights(
      [{ vaultId: 'A' }, { vaultId: 'C' }],
  ...
  it('some zero weights with more elements', () => {
    const spareWeights = getSpareWeights(
      [{ vaultId: 'A' }, { vaultId: 'C' }, { vaultId: 'D' }, { vaultId: 'E' }],
      [{ distribution: 50 }, { distribution: 30 }, { distribution: 0 }, { distribution: 20 }],
    );
    expect(spareWeights).toEqual([
      ['A', 50],
      ['C', 30],
