...
export const MintBurnFormProvider: React.FunctionComponent<MintBurnFormProviderProps> = ({
  ...
  const defaultFixedLow =
    position?.fixedRateLower.toNumber() ?? defaultValues.fixedLow ?? undefined;
  ...
  const [liquidityAction, setLiquidityAction] =
    useState<MintBurnFormLiquidityAction>(defaultLiquidityAction);
  ...
  const isTradeVerified =
    !isUndefined(mintMinimumMarginRequirement.result ?? undefined) &&
  ...
  const isAddingLiquidity =
    ...
    liquidityAction === MintBurnFormLiquidityAction.ADD;
  const isAddingMargin =
    mode === MintBurnFormModes.EDIT_MARGIN && marginAction === MintBurnFormMarginAction.ADD;
  const isRemovingLiquidity =
    mode === MintBurnFormModes.EDIT_LIQUIDITY &&
    liquidityAction === MintBurnFormLiquidityAction.BURN;
  const isRemovingMargin =
    mode === MintBurnFormModes.EDIT_MARGIN && marginAction === MintBurnFormMarginAction.REMOVE;
  ...
  const notionalChange = useCallback(
  ...
  const validateNewPosition = async () => {
    ...
    if (
      !isUndefined(minimumRequiredMargin) &&
      !isUndefined(margin) &&
  ...
  return <MintBurnFormCtx.Provider value={value}>{children}</MintBurnFormCtx.Provider>;
};
